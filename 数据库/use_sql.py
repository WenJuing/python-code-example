...
class useSql():
    ...
    def __init__(self, host, user, passwd, db):
        ...
        self.passwd = passwd
        self.db = db
        self.conn = pymysql.connect(host, user=user, passwd=passwd, db=db)
...
host = useSql('localhost', 'root', '123123', 'py_sql')
# 生成100条数据
words = list(string.ascii_letters)
for i in range(100, 200):
    random.shuffle(words)
    host.insert_many('student', [(i, "".join(words[:5]), random.randint(0, 100))])
host.select_all('student')
host.close_all()
